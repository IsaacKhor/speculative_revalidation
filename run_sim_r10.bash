...
xmake r sim -s8 --parallel=$njobs --capacity=2048 \
    -itraces/sim/wm_u.bin.zst \
    --rv-mode=never \
    --cache-type={lru,gdsf,sieve,arc,fifo} \
    --csvout=results/r${runnum}i${iter}_baseline_u.csv
...
xmake r sim -s8 --parallel=$njobs --capacity=2048 \
    ...
    --evict-expired=false \
    --cache-type={lru,gdsf,sieve,arc,fifo} \
    --csvout=results/r${runnum}i${iter}_baseline_keep_u.csv
...
xmake r sim -s8 --parallel=$njobs --capacity=2048 \
    ...
    --cache-type={lru,gdsf,sieve,arc,fifo} \
    --ml-conf-thres={1,2,3,4} \
    --csvout=results/r${runnum}i${iter}_oracle_u.csv
...
# ml models, wm
xmake r sim -s8 -p$njobs -c2048 \
    -itraces/sim/wm_u.bin.zst \
    --rv-mode=ml \
    --cache-type={lru,gdsf,sieve,arc,fifo} \
    --ml-model-path=models/v${runnum}_wm_rfc.onnx \
    --ml-conf-thres={1,0.95,0.9,0.85,0.8,0.75,0.7,0.68,0.66} \
    --csvout=results/r${runnum}i${iter}_ml_wm_rfc_u.csv
